...
namespace Mawenzy
{
    ...
    internal class BrowserLVContextMenuWrapper : NativeWindow
    {
            ...
            if (parent != null && br.FileView.SelectedOrder.Count > 0)
            {
                ...
                if ((Control.ModifierKeys & Keys.Alt) == 0)
                {
                    if (shellItem.IsFolder)
                        System.Diagnostics.Process.Start(ShellItem.GetRealPath(shellItem));
        ...
        void FileView_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            {
                // JLB show system context menu by passing mouse event to desktop.
                // ISSUE #2
                // TODO focus on sys context menu somehow.
                int pos = e.Y << 16 | e.X;
                PostMessage(UberdashMain.DefView, 0x205, 2, pos);
            }
            contextMenuVisible = false;
        }
